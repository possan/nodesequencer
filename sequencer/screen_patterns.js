...
PatternsScreen = function(opts) {
	...
	return {
		...
		activate: function(host) {
			_host = host;
			_host.displaybumper.setMessage('OLD PATTERN QUEUE');
		},
...
NewPatternsScreen = function(opts) {
	...
	var update = function() {
		...
		for( var t=0; t<4; t++ ) {
			...
			if( cued != -1 ) {
				...
				if ( x2 >= 0 && x2 < 4 && b == 0 )
					_host.display.leds[C.Leds.PAD0+(t*4)+x2] = !_host.display.leds[C.Leds.PAD0+(t*4)+x2];
			}
			else  if( pos != -1 ) {
				var x1 = pos - _host.state.currentpattern;
				if ( x1 >= 0 && x1 < 4 && b == 0 )
					_host.display.leds[C.Leds.PAD0+(t*4)+x1] = !_host.display.leds[C.Leds.PAD0+(t*4)+x1];
			}
		}
		_host.display.lcdClear();
	...
	return {
		handleEvent: function(event) {
			switch(event.type) {
				case C.Events.BUTTON_CLICK: handleButton(event.button); break;
				case C.Events.UI_ACTIVATE: activate(event.host); break;
				case C.Events.UI_UPDATE: update(); break;
			}
		}
	};
};
...
exports.registerScreens = function( repo ) {
	repo.push( { name: 'mode2', factory: function(){ return new NewPatternsScreen2(); } } )
	repo.push( { name: 'mode2', factory: function(){ return new NewPatternsScreen(); } } )
};
