...
MonoSynthScreen = function() {
	...
	var _handleButton = function(id) {
			...
			if( id == C.Keys.KNOB0_UP || id == C.Keys.KNOB0_DN  ) { 
				...
				if( id == C.Keys.KNOB0_UP ) note ++;
				if( id == C.Keys.KNOB0_DN ) note --;
	...
	var _handleEvent = function( event ){
		switch(event.type) {
			...
			case C.Events.BUTTON_CLICK: 
				if( event.button == C.Keys.KNOB2_UP ) _host.state.currenttrack = Utils.addmod( _host.state.currenttrack, 1, 0, 15 );
				if( event.button == C.Keys.KNOB2_DN ) _host.state.currenttrack = Utils.addmod( _host.state.currenttrack, -1, 0, 15 );
				if( event.button == C.Keys.KNOB2_UP || event.button == C.Keys.KNOB2_DN ) _activate(_host);
				break;
		}
